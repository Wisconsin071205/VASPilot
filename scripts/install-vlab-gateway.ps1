...
& scp -P $Port @sshBase $source "$($target):$temporary"
if ($LASTEXITCODE -ne 0) { throw "上传 Workspace Gateway 失败。" }
& ssh -p $Port @sshBase $target "mkdir -p ~/.huwei-agent/workspaces ~/bin && chmod 700 ~/.huwei-agent ~/.huwei-agent/workspaces 2>/dev/null || true && install -m 700 $temporary ~/bin/huwei-workspace-gateway && rm -f -- $temporary"
...
if (-not $SkipDoctor) {
    ...
    & ssh -p $Port @sshBase $target "~/bin/huwei-workspace-gateway doctor"
